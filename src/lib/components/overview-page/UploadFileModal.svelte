<script lang="ts">
    ...
    function onDragEnter(e: DragEvent) { e.preventDefault(); dragDepth += 1; dragActive = true; }
    function onDragOver(e: DragEvent)  { e.preventDefault(); /* keeps drop enabled */ }
    function onDragLeave(e: DragEvent) { e.preventDefault(); dragDepth = Math.max(0, dragDepth - 1); if (dragDepth === 0) dragActive = false; }
    ...
    {#snippet body()}
        ...
        {#if pickedName}
            ...
            <div class="dz-picked-left">
                ...
                <div class="dz-picked-name">{pickedName}</div>
                {#if pickedSize !== null}
                    <div class="dz-picked-size">{fmtBytes(pickedSize)}</div>
                {/if}
                </div>
            </div>
            <button class="dz-clear" onclick={clearPicked} aria-label="Clear file">✕</button>
            </div>
            <p class="dz-sub">Click to choose a different file</p>
        {:else}
            <Icon icon="tabler:upload" class="dz-icon" />
            <p class="dz-title"><strong>Drag & drop</strong> your file here</p>
            <p class="dz-sub">or click to browse</p>
            <p class="dz-accept">Accepted: {exts.join(", ")}</p>
        {/if}
        </div>
        ...
        />
        <button class="primary" onclick={pickFile}>
            <Icon icon="tabler:folder-open" class="btn-ico" />
            Browse files
        </button>
        ...
        <div class="chip">
            ...
            <span class="chip-name">{pickedName}</span>
            {#if pickedSize !== null}
                <span class="chip-size">({fmtBytes(pickedSize)})</span>
            {/if}
            <button class="chip-x" aria-label="Clear selected file" onclick={clearPicked}>✕</button>
        </div>
        {/if}
...
<style>
  ...
  .file-pill {
    ...
    background: #252525; border: 1px solid #333; border-radius: 999px;
    padding: 6px 10px;
  }
  .file-icon { font-size: 1.1rem; }
  .file-name { font-weight: 600; }
  .file-size { opacity: .7; }
  .clear { all: unset; cursor: pointer; padding: 0 6px; border-radius: 6px; }
  .clear:hover { background: #2a2a2a; }
  .dim { opacity: .7; margin-top: .5rem; }

  .dz-icon { font-size: 44px; display: block; margin: 0 auto 6px; opacity: 0.9; }
  .dz-title { margin: 6px 0 2px; }
  .dz-sub { margin: 0 0 6px; opacity: .85; }
  .dz-accept { margin: 0; font-size: .9rem; opacity: .65; }
  ...
  .dz-clear:hover { background: #2a2a2a; }
  .dz-picked-size { opacity: .75; }
  .dz-title { margin: 6px 0 2px; }
  .dz-sub { margin: 0 0 6px; opacity: .85; }
  .dz-accept { margin: 0; font-size: .9rem; opacity: .65; }
  ...
  .primary { background: var(--fill-01); color: var(--background-primary); }
  .primary:hover { background: var(--fill-02); }
  ...
  .chip {
    ...
    background: #242424; border: 1px solid #3a3a3a; border-radius: 999px;
  }
  .chip-ico { font-size: 16px; }
  .chip-name { max-width: 360px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .chip-size { opacity: .7; }
  .chip-x { all: unset; cursor: pointer; padding: 4px 6px; border-radius: 6px; }
  .chip-x:hover { background: #2a2a2a; }
