<script lang="ts">
	...
	import type { Contributor, Contacts } from '$lib/metrics';
	import {
	...
	async function generate_summaries() {
		...
		const unlisten_progress = await listen('summary-progress', (event) => {
			const { email, summary } = event.payload as { email: string; summary: string };
			summaries_store.update((store) => {
				const repo_summaries = store.get(repo_path) || new Map();
		...
		const key_set = await invoke('check_key_set');
			...
			if (!key_set) {
				...
				error_message = 'Please set a valid Gemini API key in Settings to generate summaries.';
				loading = false;
				unlisten_total();
				unlisten_progress();
				return;
	...
	function get_email(user: Contributor): string | undefined {
		if (user.contacts) {
			...
			} else if (typeof user.contacts === 'object' && user.contacts !== null) {
				if ('Email' in user.contacts) {
					return (user.contacts as { Email: string }).Email;
				} else if ('EmailList' in user.contacts && Array.isArray((user.contacts as any).EmailList) && (user.contacts as any).EmailList.length > 0) {
	...
	let repo_summaries = $derived($summaries_store.get(repo_path));
	let people_with_analysis = $derived(
		contributors.map((user: Contributor) => {
			const num_commits = get_user_total_commits(user);
			const scaling_factor = calculate_scaling_factor(num_commits, commit_mean, sd);
			let analysis = '';
			const email = get_email(user);

			if (repo_summaries) {
				if (email) {
					analysis = repo_summaries.get(email) || 'No summary available';
				} else {
...
<main class="container">
	...
	{/if}
	{#if !loading}
		{#if repo_summaries && repo_summaries.size > 0}
