    ...
    switch (source_type) {
        ...
        case 1: // GitLab
            try {
                ...
                if (typeof repo_with_git === "undefined") {
                    // Should not happen if length > 0 check passed, but defensive
                    throw new Error("Could not determine GitLab project name.");
                }
                let repo = repo_with_git.endsWith(".git")
                    ? repo_with_git.slice(0, -4)
                    : repo_with_git;
                ...
                const gitlab_name_part_regex =
                    /^[a-zA-Z0-9](?:[a-zA-Z0-9_.\-+]*[a-zA-Z0-9])?$/; // Corrected regex
                const noDoubleHyphen = /--/;

                owner.split("/").forEach((segment) => {
                    if (
                        !gitlab_name_part_regex.test(segment) ||
                        noDoubleHyphen.test(segment)
                    ) {
                        throw new Error(
                            `Invalid GitLab group/owner segment: "${segment}".`
                        );
                    }
                });
                if (
                    !gitlab_name_part_regex.test(repo) ||
                    repo.length === 0 ||
                    repo.endsWith(".") ||
                    noDoubleHyphen.test(repo)
                ) {
        ...
        case 2: // Local File
            ...
            if (!repo) {
                // Should not happen if segments.length > 0 initially
                throw new Error(
                    "Could not determine file/directory name from local path."
                );
            }
